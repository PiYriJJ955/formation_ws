...
“停止并保存”右侧的“角度对比图”会打开一个弹窗，读取当前选中模块的组均值，
把原始测量角和按当前拟合函数换算后的角度画在同一张图里：橙方块为原始测量角，
蓝圆点为拟合后角度，灰竖线为原始逐帧的最小–最大范围（需要时才读取
`observations.csv`），灰虚线为理想 45° 参考线。默认 x 轴是实际角（参考），
可切换到原始测量角；弹窗还支持导出对比数据 CSV。三种曲线可以分别开关，
逐帧范围默认关闭，避免个别帧的离群值压扁 y 轴。
