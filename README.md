...
上方四个页签是“扫描与连接 / 编队算法 / 小车定位图 / 参数调整”。第二页的车辆选择与第一页面
...
自定义配置按内场、外场分别保存在控制台设置中，切换档案不会丢失编辑结果，可恢复所选档案默认值。地图立即更新；
...
保存前会校验 YAML、基站数量、ID、坐标和几何关系；参数文件保存只在主机完成，不会自动提交或上传。
保存后当前会话的定位图也会采用文件中的基站坐标。
确认参数后请在主机手动 `git commit` / `git push`，再点击 Git 同步将发布版本更新到车辆。
