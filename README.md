...
按住行驶、松键停车；离开控制区、切换页签或关闭弹窗后需重新启用。线速度和角速度与第一页共用并记忆。

路径模式按以下步骤操作：

1. 连接实时监视，让领航车静止约 0.4 秒以建立航向参考。点击“使用当前位置为起点”，更新第一行坐标。
2. 每行填写一个 `X, Y`（UWB 地图坐标，单位米），按顺序连接成折线；至少两点，相邻点间隔至少 0.15 m。
   例如 `2.0, 2.2` → `3.0, 2.2` → `3.0, 2.8` 表示前进后转弯。示例需按实际起点调整。
3. 点击“预览路径”，在地图上检查紫色参考线。巡航默认 0.10 m/s，预瞄默认 0.40 m（可调 0.3–0.5 m）。
4. 点击“开始”。如需整队跟随，先在第二页使能跟随，再开始领航路径；单车测试可保持跟随禁用。
5. “暂停”保留进度，“继续”由当前位置继续跟踪，“停止”结束本条路径。关闭弹窗会暂停路径。
...
路径、巡航速度、预瞄距离和模式选择会保存。每次开始在 Master 主机工作空间的 `logs/leader_tracking/`
