...
点击状态可查看本机接收地址、推送凭据和已填入实际 IP 的推送命令；连接窗口提供“复制地址”“复制用户名”“复制密码”三个按钮，多网卡时列出各个可用地址。
...
任务，释放串口；若检测到车端已有底盘进程，列表显示“已有底盘运行”，并提示先停止该任务，释放串口后再点击“启动已连接底盘”重试。
...
页签包含“扫描与连接 / IOT 实时监视 / 编队算法 / 小车定位图 / IOT 数据提取”。IOT 页面可选择车辆并配置串口，
独立记住，Ctrl / Shift 多选参与车辆。列表同时显示 UGV 名称、IP、`UGV_ID`、SSH 和启动状态。
IOT 页面顶部支持选择车辆、识别 IOT UID 和串口，并按六个双向车边框显示距离与角度。没有 LinkTrack 的车辆可留空，仅配置 IOT；
ugv3 的左右 IOT 按 UID 分别配置。详见 [IOT 端口与实时监视](docs/uwb-iot.md#控制台端口识别与实时图表)。

需要独立做模块角度标定时运行 `python3 scripts/iot_calibration.py`。该窗口直接读取
主控制台 `settings.json`，选择一条有向 IOT 链路后按 -60–60°（每 5°）逐组静止采集
60 秒，保存逐帧 CSV、均值、拟合图和车端 bag；历史记录、CSV/ZIP 导出及复制拟合结果
见 [独立 IOT 角度拟合测试](docs/iot-calibration.md)。
...
2. 新车先点击“立即同步 Git 仓库（丢弃车端修改）”：自动登录、克隆 / 快进更新并按需编译，列表显示进度，
...
`config/uwb/indoor.yaml`（内场）。选择后，编辑器支持修改、增加和删除基站；点击保存会同时写入所选档案和
`config/final_localization.yaml` 的基站字段，避免当前档案、界面和基础配置不一致。配置会在下次启动定位时上传到车辆。
两个档案的解算参数仍以 `config/final_localization.yaml` 为基础。内场档案目前使用 6.4 × 4.4 米布局作为初值，请在控制台中换成内场实测坐标；外场档案当前为 23.5 × 28 米布局。
两套布局均为 ID 0 / 4 / 1 位于下边、3 / 5 / 2 位于上边，基站高度 1.35 米，标签高度 0.25 米。
“编辑基站坐标 / 定位参数”可增加、删除或修改 ID、X、Y、Z、车载标签高度和有效残差阈值；
残差阈值默认 0.6 米。
修改选中行后点击“修改选中行”，最后保存；至少保留四个 ID 唯一且不全部共线的基站。
配置按内场、外场分别保存在对应 YAML 中，切换档案不会丢失编辑结果，可清除当前会话覆盖并重新读取档案文件。地图立即更新；
**正在运行的定位节点需要先停止再启动**，才会使用新基站坐标。

定位参数可通过配置文件直接编辑；
`src/five_ugv_uwb_localization/config/final_localization.yaml`，可调整该文件的全部参数（包括基础基站坐标）。
保存前会校验 YAML、基站数量、ID、坐标和几何关系；保存会同步当前档案的基站字段。确认参数后在主机执行
`git commit` / `git push`，再点击 Git 同步即可将发布版本更新到车辆。

第五页“IOT 数据提取”按车辆从车端 `logs/uwb_iot` 下载记录，可一键提取全部车辆、查看单车文件并下载选中文件。

启动 / 同步结果弹窗提供“复制全部”，也支持在结果文本中使用 Ctrl+C（macOS 为 Cmd+C）。
...
每次启动底盘或跟随算法前，控制台都会运行仓库根目录的
`fix_python_permissions.sh`，为 `src/**/scripts/*.py` 补齐执行权限；车端启动命令也会执行同一脚本。
...
# 按控制台同步策略丢弃车端 Git 管理文件的旧修改和未跟踪文件后更新
