...
可输入命令的 SSH shell。键盘控制和启动日志区域可以折叠，终端窗口也可正常最小化。
...
第三页地图上方的“键盘控制”使用 WASD 或方向键：W / ↑ 前进，S / ↓ 后退，A / ← 左转，D / → 右转。
连接实时监视、确认领航车底盘在线后，点击“点击启用键盘控制”，按住方向键行驶，松键停车。
每次执行最近按下的方向；线速度和角速度可在控制区调整，与第一页共用并保存上次设置。
状态栏显示本次领航车及实际输出，速度只发给该车的 `/ugvN/cmd_vel`。
离开键盘控制区、切换页签、折叠控制区或断连后停车，需重新点击启用并重新按键。
在输入框编辑文字不会驱动车辆。指令有效期 0.4 秒，界面心跳中断或退出监视还会禁用跟随。
Esc、控制区内的空格键、地图上的停车按钮和步骤 4 的停车按钮可立即禁用跟随并停领航车。
重新打开程序始终需要重新启用键盘和使能编队。
实时监视通过 Master 主机的 SSH 转发 ROS 数据，主机 GUI 无需安装 ROS。

“所有小车线速度上限”逐车显示名称、IP、可编辑上限和生效状态。修改后点击“保存并应用”，
范围为 **0–0.5 m/s**，默认 **0.15 m/s**。设置按 IP 保存，重开软件或车辆改名后保留。
领航键盘的前进、后退均受该车上限约束；跟随控制器在线更新最终输出限幅，并回读确认。
未参与本次监视的车辆在下次启动时应用。**0 只停止平移，转向仍可用**；需要停车时使用停车按钮。
首次升级应先结束本次 ROS 任务、同步各车仓库，重开 GUI 后重新启动；旧控制器会显示待确认。
以后调整上限无需重启。建议领航 0.10–0.12 m/s、跟随 0.15 m/s 起步，为追赶保留余量。
...
航向箭头以连接监视时的朝向为 +X 参考；重连监视时会重新建立参考。
...
“编辑基站坐标 / 高度”可增加、删除或修改 ID、X、Y、Z，以及车载标签高度。
