...
/*    R = R*varR;*/
    ...
    f_1_minus = f_1_hat + dt*w1_sensor;

    x_2_minus = x_2_hat+dt*v2_sensor*sin(f_2_hat);
    y_2_minus = y_2_hat+dt*v2_sensor*cos(f_2_hat);
    f_2_minus = f_2_hat + dt*w2_sensor;       

    x_3_minus = x_3_hat+dt*v3_sensor*sin(f_3_hat);
    y_3_minus = y_3_hat+dt*v3_sensor*cos(f_3_hat);
    f_3_minus = f_3_hat + dt*w3_sensor;
    ...
    A1 << 1 , 0 , dt*v1*cos(f_1_hat),
          ...
          0 , 0 , 1;
    A2 << 1 , 0 , dt*v2_sensor*cos(f_2_hat),
          0 , 1 , dt*v2_sensor*(-sin(f_2_hat)),
          0 , 0 , 1;       
    A3 << 1 , 0 , dt*v3_sensor*cos(f_3_hat),
          0 , 1 , dt*v3_sensor*(-sin(f_3_hat)),
          0 , 0 , 1;  
...
void imufusionCallback(const std_msgs::Float32::ConstPtr& msg)
{
    w1 = msg->data;
}
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber imu_sub = n.subscribe<std_msgs::Float32>("imu_fusion", 1, imufusionCallback);
