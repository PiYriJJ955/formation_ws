...
{
    // 计算参考点
    x_ref = x_2_hat + dx * sin(f_2_hat) - dy * cos(f_2_hat);
    y_ref = y_2_hat + dx * cos(f_2_hat) + dy * sin(f_2_hat);
    f_ref = (1-k_dtheta)*f_2_hat + k_dtheta*atan2(x_ref-x_3_hat,y_ref-y_3_hat);
    X_ref << x_ref, y_ref, f_ref;

    // 状态转移矩阵A和控制矩阵B
    A << 1, 0, v2 * cos(f_2_hat) * dt,
         0, 1, -v2 * sin(f_2_hat) * dt,
         0, 0, 1;
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber v2_sub = nh.subscribe<std_msgs::Float32>("/v2_sensor", 1, v2Callback);
    ros::Subscriber w2_sub = nh.subscribe<std_msgs::Float32>("/w2_sensor", 1, w2Callback);
