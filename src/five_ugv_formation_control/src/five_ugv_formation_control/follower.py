...
class ControlUnavailable(RuntimeError):
    """A controller cannot produce a fresh, feasible command this cycle."""
...
class DisplacementFollower(object):
    ...
    def __init__(self):
        ...
        rospy.loginfo("%s %s follower started; offset=(%.2f, %.2f)",
                      self.robot_name, self.algorithm, self.offset_x, self.offset_y)
        if auto_align:
    ...
    def control_step(self):
        ...
        try:
            linear, angular = self.refine_command(
                leader_pose, self_pose, (body_vx, body_vy, leader_omega), dt,
                (linear, angular))
    ...
    def refine_command(self, leader_pose, self_pose, leader_velocity, dt, seed):
        """Extension point after shared state/alignment/HOLD handling, before limits."""
        return seed
    ...
    def publish_debug(self, stamp, target_x, target_y, error_x, error_y,
        ...
        target.header.frame_id = self.leader.frame_id or "uwb_map"
